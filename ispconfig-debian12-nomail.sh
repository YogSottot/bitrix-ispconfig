...
DEBIAN_FRONTEND=noninteractive apt -y install --no-install-recommends chrony mariadb-client mariadb-server rkhunter binutils curl rsyslog gnupg2 lsb-release unzip arj nomarch lzop cabextract p7zip p7zip-full unrar lrzip daemon zip libdbd-mysql-perl php8.2 php8.2-fpm php8.2-common php8.2-gd php8.2-mysql php8.2-imap php8.2-cli php8.2-cgi php-pear mcrypt imagemagick libruby php8.2-curl php8.2-intl php8.2-pspell php8.2-sqlite3 php8.2-tidy php8.2-xmlrpc php8.2-xsl php-imagick php8.2-zip php8.2-mbstring php8.2-soap php8.2-opcache php-apcu php8.2-mcrypt pure-ftpd-common pure-ftpd-mysql fail2ban iptables nginx-light fcgiwrap patch pwgen quota quotatool wget jailkit msmtp apache2-utils ufw unattended-upgrades
...
echo -e '[mysqld]\nsql_mode=\nsync_binlog=0\ntransaction-isolation=READ-COMMITTED\nquery_cache_type=ON\ninnodb_flush_log_at_trx_commit=0\ninnodb_flush_method=O_DIRECT\ninnodb_buffer_pool_instances=1\ninnodb_strict_mode=OFF\nlow-priority-updates\nslow_query_log\nlong_query_time=29.99\nslow_query_log_file=/var/log/mysql/mariadb-slow.log\ncharacter-set-server=utf8mb4\ncollation-server=utf8mb4_unicode_ci' > /etc/mysql/mariadb.conf.d/51-server.cnf
...
# mariadb limits
echo -e 'mysql soft nofile 65535\nmysql hard nofile 65535' > /etc/security/limits.d/mysql.conf
mkdir -p /etc/systemd/system/mysql.service.d/
echo -e '[Service]\nLimitNOFILE=infinity' > /etc/systemd/system/mysql.service.d/limits.conf
systemctl daemon-reload
systemctl restart mariadb

# ISPConfig is using acme.sh now as Let's Encrypt client
